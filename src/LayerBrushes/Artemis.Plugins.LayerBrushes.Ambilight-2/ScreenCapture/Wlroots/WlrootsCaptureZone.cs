...
internal sealed class WlrootsCaptureZone : ICaptureZone
{
    ...
    public WlrootsCaptureZone(Display display, int x, int y, int width, int height, int downscaleLevel)
    {
        Display = display;
        X = x;
        Y = y;
    ...
    internal unsafe void CopyFromRgb(ReadOnlySpan<byte> rgb, int sourceWidth, int sourceHeight)
    {
        if (!NeedsUpdate)
            return;

        _updateRequested = false;
        int step = 1 << DownscaleLevel;
        ...
        lock (_lock)
        {
            fixed (byte* dst = _buffer)
            {
                ...
                {
                    int sy = Math.Min(dy * step, sourceHeight - 1);
                    byte* dstRow = dst + (dy * Stride);

                    for (int dx = 0; dx < Width; dx++)
                    {
                        int sx = Math.Min(dx * step, sourceWidth - 1);
                        int sourceIndex = ((sy * sourceWidth) + sx) * 3;
                        byte* pixel = dstRow + dx * 4;
                        pixel[0] = rgb[sourceIndex + 2];
                        pixel[1] = rgb[sourceIndex + 1];
                        pixel[2] = rgb[sourceIndex];
                        pixel[3] = 255;
                    }
                }
            }
