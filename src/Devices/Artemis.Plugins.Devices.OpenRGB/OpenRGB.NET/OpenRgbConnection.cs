...
internal sealed class OpenRgbConnection : IDisposable
{
    ...
    private int _requestTimeoutMs = 2000;
    ...
    public void Connect(string name, string ip, int port, int timeoutMs, uint protocolVersionNumber = 4)
    {
        ...
        _requestTimeoutMs = Math.Max(timeoutMs, 2000);
        _connected = true;
    ...
    private async Task ReadLoop()
    {
        ...
        while (!_cancellationTokenSource.IsCancellationRequested && _connected)
        {
            try
            {
                ...
                {
                    try { Client.OnDeviceListUpdated(); }
                    catch { /* ignored */ }
                }
                else if (header.Command == CommandId.DetectionStarted)
                {
                    try { Client.OnDetectionStarted(); }
                    catch { /* ignored */ }
                }
                else if (header.Command == CommandId.DetectionProgressChanged)
                {
                    // Progress payloads are informational. Keep reading until packet 103
                    // confirms the detection pass is actually complete.
                }
                else if (header.Command == CommandId.DetectionEnded)
                {
                    try { Client.OnDetectionEnded(); }
                    catch { /* ignored */ }
                }
                else if (_pendingRequests.TryGetValue(header.Command, out BlockingCollection<byte[]>? pendingRequest))
                {
                    DebugDumpBuffer(dataBuffer, false, header.Command);
                    pendingRequest.Add(dataBuffer, _cancellationTokenSource.Token);
                }
            }
        ...
        where TRequest : ISpanWritable
    {
        ...
        try
        {
            ...
            // Several RGB.NET device queues may update concurrently. Serialize complete
            // packets so bytes from separate OpenRGB commands cannot interleave.
            lock (_sendLock)
    ...
    {
        var reader = new SpanReader(TakeResponse(command));
    ...
    private void SignalConnectionLost()
    {
        ...
        _connected = false;
        try { Client.OnConnectionLost(); }
        catch { /* ignored */ }
    }

    public TResult RequestWithReaderIndex<TArgument, TReader, TResult>(CommandId command, uint deviceId,
        int readerIndex, TArgument requestData, ReadOnlySpan<byte> additionalData = default)
        where TArgument : ISpanWritable
        where TReader : struct, ISpanReader<TResult>
    {
        Send(command, deviceId, requestData, additionalData);
        ...
        var reader = new SpanReader(TakeResponse(command));
        return TReader.ReadFrom(ref reader, CurrentProtocolVersion, readerIndex);
    }

    private byte[] TakeResponse(CommandId command)
    {
        if (_pendingRequests[command].TryTake(out byte[]? response, _requestTimeoutMs,
                _cancellationTokenSource.Token))
            return response;

        throw new TimeoutException($"OpenRGB did not respond to {command} within {_requestTimeoutMs} ms.");
    }
