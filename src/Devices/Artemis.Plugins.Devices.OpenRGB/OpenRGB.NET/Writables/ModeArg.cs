...
internal readonly record struct ModeArg(Mode Mode) : ISpanWritable
{
    public int Length
    {
        get
        {
            ...
            size += sizeof(uint) * (Mode.ProtocolVersion.SupportsStableDeviceIds ? 8 : 9);// Value (pre-v6), SpeedMin, SpeedMax, ColorMin, ColorMax, Speed, Direction, ColorMode, Flags
    ...
    public void WriteTo(ref SpanWriter writer)
    {
        ...
        if (!Mode.ProtocolVersion.SupportsStableDeviceIds)
