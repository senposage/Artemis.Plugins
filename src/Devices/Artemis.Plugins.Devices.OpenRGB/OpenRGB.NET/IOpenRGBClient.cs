...
public interface IOpenRgbClient
{
    ...
    /// <summary>
    ///     Triggered when the SDK connection is unexpectedly lost.
    /// </summary>
    event EventHandler<EventArgs>? ConnectionLost;

    /// <summary>
    ///     Triggered when OpenRGB starts a device detection pass.
    /// </summary>
    event EventHandler<EventArgs>? DetectionStarted;

    /// <summary>
    ///     Triggered when OpenRGB finishes a device detection pass.
    /// </summary>
    event EventHandler<EventArgs>? DetectionEnded;
    ...
    /// <summary>
    ///     Requests the current controller identifiers from the server. Protocol v6
    ///     returns stable identifiers; older protocols return their list indexes.
    /// </summary>
    uint[] GetControllerIds();
    ...
    /// <summary>
    ///     Requests a server-side device rescan. Supported by protocol v5 and newer.
    /// </summary>
    ...
    /// <summary>
    ///     Sets a mode using controller data already returned by the server.
    /// </summary>
    void UpdateMode(int deviceId, Device device, int modeId, uint? speed = null, Direction? direction = null, Color[]? colors = default);
